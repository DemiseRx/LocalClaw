...
## Gateway token stability fix

To prevent dashboard disconnects (`device token mismatch`) after restarting Pinokio:

- `scripts/render-local-config.mjs` now reuses the existing gateway token from:
  1. `LOCALCLAW_GATEWAY_TOKEN` (if explicitly set),
  2. existing `.localclaw/openclaw.local.json`,
  3. `.localclaw/gateway.token`.
- A new token is generated only when none exists.
- You can force a manual token rotation with `LOCALCLAW_ROTATE_GATEWAY_TOKEN=1`.

This keeps the gateway token stable between starts so dashboard auth, tool usage, and skills remain connected.
