...
## Main hub + local model mode

This build keeps your model endpoint local (`127.0.0.1/localhost`) while preserving OpenClaw hub-connected capabilities:

- `models.mode` remains `merge` so model config can coexist with other OpenClaw settings.
- `commands.native` and `commands.nativeSkills` are explicitly kept at `auto` to avoid disabling built-in tool/skill execution paths.
- Gateway token remains stable across reruns to prevent reconnect/auth churn.
