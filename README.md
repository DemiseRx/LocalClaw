...
- Startup now queries `/v1/models` and auto-selects an available local model, preferring Liquid LFM2.5/LFM2.0 1.2B model IDs when present.
- For LM Studio backends, LocalClaw defaults API mode to `openai-chat` for better chat/tool-call response compatibility.
- You can still force an exact model with `LOCALCLAW_MODEL`.
