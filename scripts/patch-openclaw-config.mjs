...
const searchProvider = (process.env.LOCALCLAW_WEB_SEARCH_PROVIDER ?? existing?.tools?.web?.search?.provider ?? 'perplexity').trim() || 'perplexity';
const perplexityApiKey = (process.env.PERPLEXITY_API_KEY ?? process.env.OPENROUTER_API_KEY ?? '').trim();
const perplexityBaseUrl = (process.env.LOCALCLAW_PERPLEXITY_BASE_URL ?? existing?.tools?.web?.search?.perplexity?.baseUrl ?? '').trim();
      ...
      search: {
        ...
        ...(searchProvider === 'perplexity' ? {
          perplexity: {
            ...
            ...(perplexityApiKey ? { apiKey: perplexityApiKey } : {}),
          },
        } : {}),
      },
    },
