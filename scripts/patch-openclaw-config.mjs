...
const discoverModel = async (baseUrl, override) => {
  ...
  const lfm = ids
    .filter((id) => /lfm2(\.5)?/i.test(id) && /(1\.2b|1_2b)/i.test(id))
...
const token = (typeof existingToken === 'string' && existingToken.trim())
  ? existingToken.trim()
  : (process.env.LOCALCLAW_GATEWAY_TOKEN ?? crypto.randomBytes(24).toString('hex'));
...
const seed = [modelId, ...discovered].filter(Boolean);
const unique = [...new Set(seed.map((x) => String(x)))];
