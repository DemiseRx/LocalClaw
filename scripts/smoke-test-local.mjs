...
const searchProxy = spawn('node', ['scripts/local-web-search-proxy.mjs'], {
  shell: true,
  stdio: 'inherit',
  env: { ...process.env, LOCAL_WEB_SEARCH_PORT: '8787', LOCAL_SEARCH_FIXTURE: 'alpha result|beta result|gamma result' },
});
...
try {
  ...
  const env = {
    ...
    LOCALCLAW_WEB_SEARCH_PROVIDER: 'perplexity',
    LOCALCLAW_PERPLEXITY_BASE_URL: 'http://127.0.0.1:8787',
  ...
  const searchRes = await fetch('http://127.0.0.1:8787/chat/completions', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify({ model: 'perplexity/sonar-pro', messages: [{ role: 'user', content: 'local search test' }] }),
  });
  const searchPayload = await searchRes.json();
  assert(searchRes.ok, 'local search proxy call failed');
  assert(Array.isArray(searchPayload.citations) && searchPayload.citations.length > 0, 'local search proxy returned no citations');


  fs.mkdirSync('.localclaw', { recursive: true });
  fs.writeFileSync('.localclaw/openclaw.local.json', JSON.stringify({
    tools: { web: { search: { provider: 'brave', perplexity: { baseUrl: 'https://api.perplexity.ai' } } } },
    gateway: { auth: { token: 'seed-token' } },
  }, null, 2));
  ...
  assert(cfg.tools?.web?.search?.provider === 'perplexity', 'web search provider should override stale brave setting');
  assert(cfg.tools?.web?.search?.perplexity?.baseUrl === 'http://127.0.0.1:8787', 'web search should use local proxy baseUrl');
